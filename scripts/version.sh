#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# version.sh — single source of truth for version management.
#
# Usage:
...
#   ./scripts/version.sh sync          Re-sync Cargo.toml to VERSION file
...
# Replace version = "OLD" with version = "NEW" in Cargo.toml,
...
cmd_release() {
  ...
  local ver
  ver="$(current_version)"
  git add "$VERSION_FILE" "$ROOT/Cargo.toml" "$ROOT/Cargo.lock"
